...
public interface MyRecordRepository extends CrudRepository<MyRecord, Long>{
        ...
        @Query("SELECT mr FROM MyRecord mr WHERE " +
                "MONTH(mr.dateOfTransaction) = MONTH(:givenDate) AND " +
                "YEAR(mr.dateOfTransaction) = YEAR(:givenDate) AND " +
                "(mr.bankAccount IN (:givenBankAccountList) OR :givenBankAccountList IS NULL) AND " +
                "(mr.typeTransaction NOT IN :excludedTypes OR :excludedTypes IS NULL)")
        List<MyRecord> findMyRecordsByMonthAndBankAccounts(@Param("givenDate") LocalDate givenDate,
                                                     @Param("givenBankAccountList") List<BankAccount> givenBankAccountList,
        ...
        List<ThemeSub> findDistinctThemeSubsFrom(@Param("excludedThemeSubList") List<ThemeSub> excludedThemeSubList);
